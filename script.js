...
(function () {
  ...
  // A missing or renamed cover file degrades to the placeholder slot instead of a
  // broken-image icon, so a post can reference an image before the file is added.
  window.__sudarvaCoverError = function (img, ph) {
    try {
      var slot = document.createElement('image-slot');
      slot.setAttribute('shape', 'rect');
      slot.setAttribute('fit', 'cover');
      slot.setAttribute('placeholder', ph || 'Cover');
      if (img && img.parentNode) img.parentNode.replaceChild(slot, img);
    } catch (e) {}
  };

  // Insight cover: a real <img> when the post has a cover — either a Sanity CDN URL or a
  // file dropped in /public/insights/ — otherwise the placeholder slot.
  function insCover(p, ph) {
    if (p && p.cover) {
      return '<img src="' + esc(p.cover) + '" alt="' + esc(p.title || '') + '" loading="lazy" ' +
        'style="width:100%;height:100%;object-fit:cover;display:block;" ' +
        'onerror="window.__sudarvaCoverError(this,\'' + esc(ph) + '\')">';
    }
  ...
  var insightsPosts = (typeof window !== 'undefined' && window.SUDARVA_INSIGHTS && window.SUDARVA_INSIGHTS.length) ? window.SUDARVA_INSIGHTS : [
    { id: 'distribution-layer', tag: 'Thesis', date: 'Jul 2026', read: '6 min', cover: '/insights/distribution-layer.jpg', title: 'Distribution is the last unbuilt layer in travel', excerpt: 'Everyone integrated payments and bookings years ago. The layer that turns a network profile into something a travel business can actually sell was left for each company to rebuild. That\u2019s the gap.',
      body: [
    ...
    { id: 'roaming-penalty', tag: 'Industry', date: 'Jun 2026', read: '7 min', cover: '/insights/roaming-penalty.jpg', title: 'The retail roaming penalty, and why travellers keep paying it', excerpt: 'A normal SIM routes home across every border. Multi-profile infrastructure connects each traveller in-market instead. The economics of that difference, explained.',
      body: [
    ...
    { id: 'built-then-shown', tag: 'Company', date: 'May 2026', read: '4 min', cover: '/insights/built-then-shown.jpg', title: 'Built, then shown: how we think about shipping', excerpt: 'We don\u2019t announce what we haven\u2019t shipped, and we don\u2019t play valuation games. A short note on the principles underneath the roadmap.',
      body: [
